...
#     topics = json.load(f)[:6]
# topics = topics[:4]+topics[5:]
...
# split_model_values=()
# for i in {1..5}; do
#     split_model_values+=("${topics_array[i]}_removed_outliers")
# done
# for i in {1..5}; do
...
split_model_values=()
for i in {4..9}; do
    split_model_values+=("trivia_cluster_${i}")
done

# split_eval_values=("${split_model_values[@]}")

for i in {4..9}; do
    split_eval_values+=("trivia_cluster_${i}")
done
...
config_file="bash_config_longform.conf"  # Base config file that contains other parameters
...
for i in "${!split_eval_values[@]}"; do
    ...
    # CUDA_VISIBLE_DEVICES=1 python -m torch.distributed.run --nproc_per_node=1 --master_port=12331 forget.py --config-name=forget_${forget_loss}.yaml split=$split_model batch_size=4 gradient_accumulation_steps=4 model_family=$model_family lr=$lr
...
#     # Update model path dynamically if needed
    ...
    # input_path="${input_path//_longform/}"
    ...
    #         CUDA_VISIBLE_DEVICES=4 python3 evaluate_util_longform.py model_family=$model_family split=$split_eval model_path=$model_path
    ...
    #                               method_name=base save_file=${split_eval}_${split_model}_${forget_loss}_${sorted_checkpoints[i]}_lora1_c2_${i}_base_result_long.csv
    ...
    CUDA_VISIBLE_DEVICES=6 python3 evaluate_util_longform.py model_family=$model_family split=$split_eval model_path=$model_path

    # Aggregate evaluation statistics
    python aggregate_eval_stat.py retain_result=$model_path/eval_results/ds_sizeFalse/eval_log_aggregated.json \
                                  ckpt_result=$model_path/eval_results/ds_sizeFalse/eval_log_aggregated.json \
                                  method_name=base save_file=${split_eval}_base_result.csv

    echo "Completed evaluation for split_eval=${split_eval}"
