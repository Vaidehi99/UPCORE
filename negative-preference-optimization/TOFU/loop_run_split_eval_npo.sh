...
config_file="bash_config_npo.conf"  # Base config file that contains other parameters
...
for i in "${!split_eval_values[@]}"; do
    ...
    # CUDA_VISIBLE_DEVICES=4 python -m torch.distributed.run --nproc_per_node=1 --master_port=12335 forget.py --config-name=forget_${forget_loss}.yaml split=$split_model batch_size=4 gradient_accumulation_steps=4 model_family=$model_family lr=$lr

    # CUDA_VISIBLE_DEVICES=4 python3 -m torch.distributed.run --nproc_per_node=1 --master_port=12338 forget.py --config-name=forget.yaml split=${split_model} batch_size=4 gradient_accumulation_steps=4 model_family=${model_family} lr=${lr}
    ...
    if [ "$checkpoints_found" = true ]; then
        ...
        for ((i = 0; i < 10; i += 1)); do
            ...
            python aggregate_eval_stat.py retain_result=$model_path/eval_results/ds_sizeFalse/eval_log_aggregated.json \
                                  ...
                                  method_name=base save_file=${split_eval}_${split_model}_${forget_loss}_${sorted_checkpoints[i]}_lora1_c2_${i}_result.csv
